...
if [[ ! -f "./scripts/flatpak/io.github.acerage.EdgeSlicer.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/io.github.acerage.EdgeSlicer.yml${NC}"
    exit 1
fi
...
if ! flatpak-builder \
    "${BUILDER_ARGS[@]}" \
    "$BUILD_DIR/build-dir" \
    scripts/flatpak/io.github.acerage.EdgeSlicer.yml; then
...
if ! flatpak build-bundle \
    ...
    "$BUILD_DIR/repo" \
    "$BUNDLE_NAME" \
    io.github.acerage.EdgeSlicer; then
...
echo -e "flatpak run io.github.acerage.EdgeSlicer"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user io.github.acerage.EdgeSlicer"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
