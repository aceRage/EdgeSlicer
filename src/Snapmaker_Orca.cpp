        ...
        for (const std::string& file : m_input_files) {
            ...
            try {
                ...
                if (is_bbl_3mf)
                {
                    ...
                    // one per filament, then the printer, and the code below indexes it on that
                    // assumption. An empty vector makes size - 1 wrap around; a single entry makes
                    // size - 2 wrap in the resize; a longer one reads current_filaments_name past
        ...
        if (m_models.size() > 0)
        {
            ...
            while(!finished_arrange)
            {
                ...
                if ((duplicate_count == 0) && (plate_to_slice == 0))
                {
                    ...
                    if (from_project)
                        for (int plate_index = 0; plate_index < plate_count; ++plate_index) {
                            // Edge #29: trailing false keeps mixed virtual IDs (do not expand).
                            plate_filament_counts[plate_index] = static_cast<int>(
                                partplate_list.get_plate(plate_index)->get_extruders_under_cli(true, m_print_config, false).size());
                        }
