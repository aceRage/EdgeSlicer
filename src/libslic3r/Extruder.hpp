...
public:
    ...
    // SEMM (m_share_extruder) reads the shared scalar so custom G-code e_retracted
    // matches retract()/unretract(). Edge keeps one static, not Orca's per-physical vector.
