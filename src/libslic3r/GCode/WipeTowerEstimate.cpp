...
WipeTowerFootprint estimate_wipe_tower_footprint(const ConfigBase &config, size_t filaments_cnt, double layer_height, double max_object_height)
{
    ...
    // Orca names this prime_tower_infill_gap; EdgeSlicer ships wipe_tower_extra_spacing.
    // Prefer an explicit Orca key on the config, never a def fallback that would hide ours.
    const double extra_spacing    = (config.option("prime_tower_infill_gap") != nullptr ? opt_float("prime_tower_infill_gap") :
                                                                                           opt_float("wipe_tower_extra_spacing")) /
                                 100.;
