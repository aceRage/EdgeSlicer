...
bool Layer::is_perimeter_compatible(const PrintRegion& a, const PrintRegion& b)
{
    ...
    return config.wall_filament             == other_config.wall_filament
        ...
        && config.gap_infill_speed.values.front() == other_config.gap_infill_speed.values.front()
    ...
    for (LayerRegionPtrs::iterator layerm = m_regions.begin(); layerm != m_regions.end(); ++ layerm) 
    	if ((*layerm)->slices.empty()) {
	        ...
	        if (layerms.size() == 1) {  // optimization
	            ...
	            // Orca #15662: Unlike the compatible regions above, the initiating region has not
