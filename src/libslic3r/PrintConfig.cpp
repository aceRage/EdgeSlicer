...
void PrintConfigDef::init_fff_params()
{
    ...
    def->tooltip = L(
        ...
        "set appropriately. EdgeSlicer ensures that adaptive_bed_mesh_min/adaptive_bed_mesh_max values do not exceed these min/max "
    ...
    */
    ...
    def->tooltip = L("After a tool change, the exact position of the newly loaded filament inside "
                     ...
                     "Before purging the print head into an infill or a sacrificial object, EdgeSlicer will always prime "
    ...
    def->tooltip = L("Connect an infill line to an internal perimeter with a short segment of an additional perimeter. "
                     ...
                     "EdgeSlicer tries to connect two close infill lines to a short perimeter segment. If no such perimeter segment "
    ...
    def->min = 1;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloats { 60.0 });
    ...
    def->tooltip = L("EdgeSlicer can upload G-code files to a printer host. This field must contain "
    ...
    def->tooltip = L("If you want to process the output G-code through custom scripts, "
                   ...
                   "and they can access the EdgeSlicer config settings by reading environment variables.");
    def->gui_flags = "serialized";
    def->multiline = true;
    def->full_width = true;
    def->height = 6;
    ...
    def->tooltip = L("If enabled, the wipe tower will not be printed on layers with no tool changes. "
                    ...
                    "Has no effect with smooth timelapse, which needs a tower on every layer.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
...
//    def->min = 0;
    ...
    def->tooltip  = L("Print odd-numbered walls shifted by half a layer height so layers interlock, improving layer adhesion and part strength. "
                      "Experimental: works with either wall generator, and requires equal first layer and layer heights, matching top surface / outer wall line widths, "
                      "and spiral vase mode off. Adaptive layer height is not supported.");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
...
void PrintConfigDef::init_extruder_option_keys()
{
    ...
    m_extruder_option_keys = {
        ...
        // Snapmaker: flow-variant (already defined; resized with nozzle_diameter)
        "nozzle_volume_type"
    };
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L("This version of EdgeSlicer may not understand configurations produced by the newest OrcaSlicer versions. "
...
ReadWriteSlicingStatesConfigDef::ReadWriteSlicingStatesConfigDef()
{
    ...
    def->tooltip = L("Position of the extruder at the beginning of the custom G-code block. If the custom G-code travels somewhere else, "
                     "it should write to this variable so EdgeSlicer knows where it travels from when it gets control back.");
    ...
    def->tooltip = L("Retraction state at the beginning of the custom G-code block. If the custom G-code moves the extruder axis, "
                     "it should write to this variable so EdgeSlicer de-retracts correctly when it gets control back.");
