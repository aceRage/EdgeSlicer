...
static const char* s_region_filament_keys[] = {
    ...
    "internal_solid_infill_filament_id", "top_surface_filament_id", "bottom_surface_filament_id",
};
...
static size_t num_physical_from_config(const DynamicPrintConfig& config)
{
    const auto* colours = config.option<ConfigOptionStrings>("filament_colour");
    return colours == nullptr ? 0 : colours->size();
}

static MixedFilamentManager mixed_mgr_from_config(const DynamicPrintConfig& config)
{
    MixedFilamentManager mgr;
    const auto* colours = config.option<ConfigOptionStrings>("filament_colour");
    std::vector<std::string> colors = colours ? colours->values : std::vector<std::string>{};
    if (MixedFilamentManager::auto_generate_enabled())
        mgr.auto_generate(colors);
    if (const auto* defs = config.option<ConfigOptionString>("mixed_filament_definitions"))
        mgr.load_custom_entries(defs->value, colors);
    return mgr;
}
...
{
    const MixedFilamentManager mgr = mixed_mgr_from_config(config);
    return mgr.is_mixed(unsigned(slot_0based + 1), num_physical_from_config(config));
}

// `slots` with every mixed slot replaced by its component filaments (0-based).
static std::vector<int> physical_slots_of(const std::vector<int>& slots, const DynamicPrintConfig& config)
{
    MixedFilamentManager mgr = mixed_mgr_from_config(config);
    const size_t num_physical = num_physical_from_config(config);
    if (num_physical == 0)
        return slots;
    ...
    for (int slot : slots)
        ids.push_back(slot + 1);
    mgr.expand_virtual_extruder_ids(ids, num_physical);

    std::set<int> physical;
    for (int id : ids)
        if (id > 0)
            physical.insert(id - 1);
    return std::vector<int>(physical.begin(), physical.end());
}
...
FilamentCompaction build_filament_compaction(const Model& model, const DynamicPrintConfig& config, size_t namespace_size)
{
    ...
    // Only a plate reaching past the namespace is packed.
    ...
    // A plate that already uses a dense prefix needs no renumbering.
...
// permutation of them, so it has to be applied to every triangle at once.
...
// Edge: mixed metadata lives in mixed_filament_definitions, not filament_is_mixed arrays.
static const std::vector<std::string>& filament_indexed_keys()
{
    static const std::vector<std::string> keys = [] {
        std::vector<std::string> out = Preset::filament_options();
        for (const char* extra : { "filament_colour", "filament_multi_colors", "filament_settings_id",
...
static void remap_mixed_filament_definitions(DynamicPrintConfig& config, const FilamentCompaction& compaction)
{
    auto* defs = config.option<ConfigOptionString>("mixed_filament_definitions");
    if (defs == nullptr || defs->value.empty())
        return;

    MixedFilamentManager mgr = mixed_mgr_from_config(config);
    for (MixedFilament& mf : mgr.mixed_filaments()) {
        mf.component_a = unsigned(std::max(1, compacted_1based(compaction, int(mf.component_a))));
        mf.component_b = unsigned(std::max(1, compacted_1based(compaction, int(mf.component_b))));
        if (!mf.gradient_component_ids.empty()) {
            std::vector<unsigned int> ids = MixedFilamentManager::decode_gradient_component_ids(mf.gradient_component_ids, 0);
            for (unsigned int& id : ids)
                id = unsigned(std::max(1, compacted_1based(compaction, int(id))));
            mf.gradient_component_ids = MixedFilamentManager::encode_gradient_component_ids(ids);
        }
    }
    defs->value = mgr.serialize_custom_entries();
}
...
void apply_filament_compaction(DynamicPrintConfig& config, const FilamentCompaction& compaction)
{
    ...
    // Mixed virtual slots live after the physical colour list. Gather only physical slots.
    std::vector<int> physical_slots;
    physical_slots.reserve(compaction.slot_of_tool.size());
    for (int slot : compaction.slot_of_tool)
        if (slot >= 0 && size_t(slot) < old_count)
            physical_slots.push_back(slot);
    const size_t new_count = physical_slots.size();
    ...
    for (const std::string& key : filament_indexed_keys()) {
        ...
        // Only gather what is genuinely one-entry-per-filament.
        if (vec->size() != old_count)
            continue;
        const std::unique_ptr<ConfigOption> source(vec->clone());
        for (size_t tool = 0; tool < new_count; ++tool)
            vec->set_at(source.get(), tool, size_t(physical_slots[tool]));
        vec->resize(new_count);
    }

    remap_mixed_filament_definitions(config, compaction);

    // flush_volumes_matrix is one filament x filament block (row = source, column = target) PER
    // HEAD on a multi-head printer, and a single block on single-head machines.
            ...
            for (size_t row = 0; row < new_count; ++row)
                for (size_t col = 0; col < new_count; ++col)
                    gathered[dst_base + row * new_count + col] =
                        matrix->values[src_base + size_t(physical_slots[row]) * old_count +
                                       size_t(physical_slots[col])];
        }
        matrix->values = std::move(gathered);
    }
    // flush_volumes_vector carries a fixed group of entries per filament.
        ...
        for (size_t tool = 0; tool < new_count; ++tool)
            for (size_t entry = 0; entry < per_filament; ++entry)
                gathered[tool * per_filament + entry] =
                    vector->values[size_t(physical_slots[tool]) * per_filament + entry];
