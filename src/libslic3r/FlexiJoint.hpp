...
struct FlexiJointParams
{
    ...
    // the pair's orientation within the plane changes. Double ring and ball & socket are
    // solids of revolution about n, so rotating them changes nothing and they IGNORE this
    // value. 0-180 is the whole range: a chain link at 180 degrees presents the same two ring
    // planes as at 0. Default 0 - which is also what an older 3MF without the field loads as.
    float rotation{ 0.f };
