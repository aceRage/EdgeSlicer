...
// Always go through merge(): assigning min/max without setting `defined` would
// make the next merge() treat the box as empty and drop the first vertex.
BoundingBoxf3 its_referenced_bbox(const indexed_triangle_set &its)
{
    BoundingBoxf3 bb;
    for (const stl_triangle_vertex_indices &t : its.indices)
        for (int k = 0; k < 3; ++k)
            bb.merge(its.vertices[t(k)].cast<double>());
    return bb;
}

json vec3_to_json(const Vec3d &v) { return json::array({v.x(), v.y(), v.z()}); }
...
json inspect_layer(const ModelVolume                                          &mv,
                   const FacetsAnnotation                                     &fa,
...
const std::vector<std::pair<EnforcerBlockerType, std::string>> &mmu_states()
{
    static std::vector<std::pair<EnforcerBlockerType, std::string>> s = [] {
        ...
        // Virtual mixed-filament IDs are not separate paint layers; enumerate
        // the TriangleSelector extruder states only (extruder_1 .. ExtruderMax).
...
void inspect_to_json(const Model &model, const std::vector<std::string> &source_paths, std::ostream &out)
{
    ...
    root["note"]    = "Coordinates are mesh-local (each volume's own frame). Paint gizmos operate in this frame.";
