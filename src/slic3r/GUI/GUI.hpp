...
// Reports preset files that could not be parsed and were moved aside rather than deleted.
// Drains the accumulator, so it is safe (and a no-op) to call when nothing was quarantined.
