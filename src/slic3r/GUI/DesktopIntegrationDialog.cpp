...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/EdgeSlicer%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (create_desktop_file(desktop_path, desktop_file))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. EdgeSlicer desktop file was probably created successfully."));
        }
    }
