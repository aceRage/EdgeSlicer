...
## 2f. Stage 5 merged into `feat/ultra-preferences` (2026-09-05): the over-support keys act per part AND the walls follow them

Branch `feat/stage5-merge`, cut from `feat/ultra-preferences` @ `4afeb90fb4`, merging
`feat/support-sets-stage5` @ `d302501f7b` (cut from `4bc7522940`). One textual conflict, in
`src/libslic3r/PrintObject.cpp`; the semantic merge was the work.

### What the two sides did to the same function

Stage 5 moved `over_support_surfaces` / `_flow` / `_speed` from `PrintObjectConfig` to
`PrintRegionConfig` (2e deviation 2) and taught `detect_surfaces_type` to read the switch per
region. Meanwhile `feat/ultra-preferences` had merged `feat/over-support-walls` (`b0a44585c2`),
which factored the whole slice-time reconstruction into **one** predicate,
`PrintObject::over_support_settings()`, shared by the bottom-face classifier and a NEW wall
classifier (`build_over_support_below()`, `PerimeterGenerator::over_support_region()`, the
`erOverSupportPerimeter` role, `GCode::_extrude` reading the two G-code keys for both new roles).

### How the conflict was resolved

**(a) The three keys now read from the REGION config on every path.**

- `over_support_settings()` keeps everything the OBJECT decides - `has_support()`, the non-zero
  `support_top_z_distance`, the per-generator bridgeable rule, the enforcer and blocker
  projections, `active` - and nothing else. Its `on` flag, which used to read
  `m_config.over_support_surfaces`, now reads a new `PrintObject::any_region_over_support()`: the
  OR of the parts' switches. That is only an "is this feature in play on this object at all" test,
  and it is what decides whether the shared reconstruction is built; it is never the per-part
  answer.
- `detect_surfaces_type` ANDs the object-wide result with
  `printing_region(region_id).config().over_support_surfaces` inside the region loop, which is
  Stage 5's arrangement expressed against the base's struct. `over_support.active` already folds in
  "an auto type, or at least one enforcer somewhere", so the region-level `active` is one `&&`.
- The walls follow the same rule at the other consumer:
  `PerimeterGenerator::over_support_active()` additionally requires
  `config->over_support_surfaces` - `config` being the generator's own `PrintRegionConfig`. A part
  that did not ask therefore takes `traverse_loops` / `traverse_extrusions`'s original code path
  verbatim, the same stand-down a build with the feature off takes. `build_over_support_below()`
  is unchanged: the reconstruction is object-wide work, built as soon as any part asks.
- `PrintObject::invalidate_state_by_config_options`'s `enable_support` branch also asked
  `m_config.over_support_surfaces`; it now asks `any_region_over_support()`. (The
  `opt_key == "over_support_surfaces"` -> `posSlice` entry needed no change, and
  `discover_horizontal_shells` already reads `region_config` from Stage 5.)
- `GCode.cpp` needed **no** change: its `m_config` is a `FullPrintConfig` and
  `extrude_perimeters` / `extrude_infill` both `m_config.apply(region.config())` before extruding,
  so both new roles pick up the part's flow and speed for free.

**(b) The bridgeable rule: the base's version was kept, Stage 5's carried copy dropped.**
`4afeb90fb4` already contains the correction 2e deviation 3 describes - classic tree only uses the
per-face `max_bridge_length` test, `normal(auto)` and the organic tree stand down under
`bridge_no_support` - and it lives inside `over_support_settings()`. Stage 5's `22039203ea` carried
its own copy of the same fix in the classifier hunk; that half of the hunk was discarded rather
than duplicated. The base's `[OverSupport]` case that walks the three generator paths
("the tree generators refuse bridges each in their own way") is kept, so the suite is 14 cases
rather than Stage 5's 13. **2e's "what is left" item about `feat/ultra-preferences` needing
deviation 3's correction is closed by this merge: the branch already had it.**

### The gates, re-run on the merge

Baseline: a build of `feat/ultra-preferences` @ `4afeb90fb4` itself, installed to a scratch prefix.
Candidate: this merge, likewise. Both driven on an isolated `--datadir`. Build clean, 0 `error C`,
0 `error LNK`, configure reports `libcurl will have HTTP/2`.

- **Off-mode tolerance gate** - green on all 20 cases, and every segment count is the number 2e
  records, to the segment (`normal_grid` 4466 ... `handy_bunny_tree_organic` 708354).
- **`--gate groups --baseline-has-groups`** - green on all 13 cases, every number 2e records
  reproduced: `config_rows=0` everywhere, `identical=61/61` (41/41) and `segments=100.0000%` on the
  nine cases that must match, and on the four that must differ `group_parts_ironing` 91.7413% with
  Ironing 99.7 % / 0.3 % (2837.0 mm / 9.4 mm), `_tree_organic` 98.8077% with 100.0 % / 0.0 %
  (2461.4 mm), `_tree_classic` 98.8293% with 100.0 % / 0.0 % (2775.2 mm), and **`part_over_support`
  61.4471% with "Bottom surface over support" 100.0 % / 0.0 % (614.9 mm against 0.0 mm)** - the
  per-part claim of this stage, measured against a baseline that carries the walls pass.
- **Catch2** - `[ironing]` 4 cases / 48 assertions, `[OverSupport]` **14 / 244** (Stage 5's 13 / 234
  plus the base's three-generator case), `[SupportGroups]` 14 / 63, `[support_groups]` 18 / 266,
  `[TreeSupportDeterminism]` 5 / 39, `libslic3r_tests` 599 cases / 53 750 assertions with the same
  two `[!shouldfail]`.

### The shipped CLI, on `tests/supportgroup_test.3mf`

tree(auto) / `support_style = default` (organic), `max_bridge_length = 10`,
`support_top_z_distance = 0.2`, `bridge_no_support = 0`, Arachne, `outer_wall_speed = 60`.
Sliced pinned to one CPU, isolated `--datadir`.

- **Off mode is byte-identical.** With every `over_support_*` key stripped from
  `Metadata/model_settings.config`, `over_support_surfaces = 0` in the project settings **and the
  file's support-group data removed** (so K == 1, which is what off mode means), the candidate and
  the user's live install produce, on both plates, the same byte count, the same line count and
  exactly one differing line - the `; generated by` header. Normalised SHA-256 equal on both plates.
- **The group half of the same file, isolated.** Leaving the "PVA Interface" group in place but
  setting its `support_ironing` to 0 is **also** byte-identical on both plates
  (plate 1 `c1ce21d6...`, plate 2 `5077b485...`). Leaving that group's `support_ironing = 1`,
  plate 1 gains 15 `;TYPE:Ironing` blocks and 7 `;TYPE:Support interface` blocks and nothing else -
  Stage 5's per-group ironing on the organic tree, on a project whose object-wide `support_ironing`
  is 0. So the off-mode delta on this file is the group feature acting, not the over-support merge.
- **On mode**, switch on everywhere, `over_support_flow = 1`, `over_support_speed = 0`:
  plate 1 **144** "Wall over support" blocks (894 moves, 428.32 mm) and **33** "Bottom surface over
  support" blocks (924 moves, 405.10 mm), plate 2 **12** and **2** - and the `4afeb90fb4` baseline
  produces exactly the same four numbers. The user's older live install produces 132 / 3 on plate 1
  (12 / 2 on plate 2, unchanged). **That gap is `4afeb90fb4`, not this merge:** the file's
  `bridge_no_support` is 0, so the half of that commit about standing down never fires, but its
  other half does - the per-face `max_bridge_length` test now applies to the CLASSIC tree only, and
  this file is an ORGANIC tree with `max_bridge_length = 10`, so faces the older build excluded as
  bridgeable are now correctly claimed. Every over-support move runs at `F=3600`, the profile's
  `outer_wall_speed`.
- **Per part, on the same file.** Object 3 has three copies of one part, 32.06 mm apart in Y. With
  the switch on **one** part only: 48 wall blocks and 11 bottom blocks, all inside
  `Y[149.51, 164.72]`. Moving the switch to a **different** part: the same 48 and 11 blocks, all
  inside `Y[181.57, 196.78]` - 32.06 mm away, **disjoint**, with nothing at all in the other part's
  band. Plate 2, where no part asked, has zero blocks of either role. The corpus case
  `part_over_support` is the same claim on a purpose-built two-part object, measured by
  `expect_feature_part`: 614.9 mm on the part that asked, 0.0 mm on its neighbour.

### What is left

- Everything 2e's own "what is left" still lists except the last item, which this merge closes:
  the seam screenshot, the preview-colour eyeball, the over-support both-on attribution
  (2e deviation 4) and the `test_over_support_surfaces.cpp` codegen artefact (2e deviation 7).
- The manual checklists of 2e were not re-run; this merge changed no GUI code.
