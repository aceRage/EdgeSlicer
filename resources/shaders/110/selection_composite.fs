...
{
    return texture2D(full_mask_texture, tex_coord + offsetPixels * full_texel_size).a;
}
...
void main()
{
    ...
    vec4 mask = texture2D(mask_texture, tex_coord);
    ...
    vec3 compositeColor = vec3(fillGray) * fillCoverage * (1.0 - edgeAlpha) +
