...
TEST_CASE("A single filament only gets a tower when one is printed anyway", "[WipeTowerEstimate]") {
    ...
    CHECK_THAT(estimate_wipe_tower_footprint(config, 1, 0.2, 100.).depth, WithinAbs(20., 1e-9));
    CHECK_THAT(estimate_wipe_tower_footprint(config, 1, 0.2, 5.).depth,
               WithinAbs(std::max(10., double(WipeTower::get_limit_depth_by_height(5.f))), 1e-9));
}
...
TEST_CASE("A rib wall squares the tower and caps the rib width", "[WipeTowerEstimate]") {
    ...
    // A wide rib on a short floor is capped at half the body depth.
    config.set_key_value("wipe_tower_extra_rib_length", new ConfigOptionFloat(0.));
    config.set_key_value("prime_volume", new ConfigOptionFloat(5.));
    config.set_key_value("wipe_tower_rib_width", new ConfigOptionFloat(30.));
    const double floor = WipeTower::get_limit_depth_by_height(5.f);
    CHECK_THAT(estimate_wipe_tower_footprint(config, 2, 0.2, 5.).depth, WithinAbs((floor / 2.) / std::sqrt(2.) + floor, 1e-9));
}
