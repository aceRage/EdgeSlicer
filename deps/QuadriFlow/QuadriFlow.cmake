...
#
# The PATCH_COMMAND then runs add_cstdint.cmake. Five of the sources we compile use
# uint8_t/uint32_t/uint64_t without including anything that declares them, and have
# only ever got away with it: MSVC and Apple clang drag <cstdint> in through other
# standard headers. libstdc++ stopped doing that in GCC 13, so the ubuntu-24.04 CI
# deps build fails with "'uint32_t' does not name a type" at loader.cpp:31, and the
# Flatpak runtime's GCC 15 prunes more of those transitive includes still. The script
# prepends the include upstream should have had. It is a cmake -P script rather than
# `git apply` + a .patch on purpose: inside a work tree git treats a git-format patch
# as repo-root-relative and silently skips every hunk (exit 0), and the --directory
# guard the other patched deps carry depends on IN_GIT_REPO, which deps/CMakeLists.txt
# defines only after this file is included. See the script for the full story.
#
...
Snapmaker_Orca_add_cmake_project(QuadriFlow
    ...
    PATCH_COMMAND ${CMAKE_COMMAND} -E copy ${CMAKE_CURRENT_LIST_DIR}/CMakeLists.txt.in ./CMakeLists.txt
          COMMAND ${CMAKE_COMMAND} -P ${CMAKE_CURRENT_LIST_DIR}/add_cstdint.cmake
    CMAKE_ARGS
